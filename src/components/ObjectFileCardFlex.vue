...
<div class="border relative border-gray-200 p-4 rounded-md h-14 items-center justify-between text-sm hover:shadow-xl">
   <!-- Decorate -->
   <div :class="[object.dir ? '': '']" class="rounded-full absolute bg-indigo-400 opacity-30 w-10 h-10 bottom-1.5 left-1.5"></div>
   ...
   <div class="flex items-center justify-between">
      <div class="flex items-center w-1/2">
         <svg v-if="object.dir" xmlns="http://www.w3.org/2000/svg" class="flex-shrink-0 h-5 w-5 mr-4 text-indigo-500" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
         </svg>
         <svg v-else xmlns="http://www.w3.org/2000/svg" class="flex-shrink-0 h-5 w-5 mr-4 text-indigo-500"  fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="inline-flex items-center space-x-1 text-xs">
         <div class="inline-flex items-center space-x-2">
            <span v-if="!object.dir">{{ formatBytes(object.size) }} |</span>
            <span>{{ formatDateModified(object.lastModified) }} </span>
         </div>
         <div class="z-10 relative">
            <svg xmlns="http://www.w3.org/2000/svg" @click="isOpenOption = !isOpenOption" class="h-5 w-5 text-gray-500 cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <!-- Options -->
            <div v-if="isOpenOption" class="inline-flex items-center justify-around absolute p-2 bg-white right-5 top-0 h-14 w-36 shadow-xl border rounded-md">
